...
def train_test_generator(dataset_name):
    ...
    y_dataset = dataset_name.replace("dataset_X", "dataset_y")
    ...
    training_set_data, test_set_data, training_set_labels, test_set_labels = train_test_split(X, integer_encoded,
                                                                                              test_size=0.7,
...
def compute_classification_thresholds(model=None, test_set=None, labels=None, clsf=None):
    ...
    for lbl, threshold in zip(labels, thresholds):
        dict_item = clsf[lbl]
        new_dict = {'precision': dict_item['precision'], 'recall': dict_item['recall'],
                    'f1-score': dict_item['f1-score'], 'support': dict_item['support'], 'threshold': threshold}
        clsf[lbl] = new_dict
...
def random_forest_kfinger(path="training/", random_number_model=1,type_factorization='CFL', k=8):
    ...
    # Create name dataset
    dataset_name = path + "datasets_X_y/" + "dataset_X_" + type_factorization + "_K" + str(k) + ".pickle"
    train_scaled_D, test_scaled_D, training_set_labels, test_set_labels, label_encoder, min_max_scaler = train_test_generator(
        dataset_name)
...
def test_reads_fusion(list_best_model=None, path='testing/', type_factorization='CFL_ICFL-20', k_value=8,
                      fingerprint_block=[]):
