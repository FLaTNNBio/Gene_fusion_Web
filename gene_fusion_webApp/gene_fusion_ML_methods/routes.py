...
def validate_files_ML():
    ...
    if execution_type == "MML_experiment":
        MML_chimeric_fingerprint_file = request.files.get("dataset_chimeric_fingerprint_file")
        MML_non_chimeric_fingerprint_file = request.files.get("dataset_non_chimeric_fingerprint_file")
        ...
        chimeric_content = MML_chimeric_fingerprint_file.read().decode('utf-8')
        non_chimeric_content = MML_non_chimeric_fingerprint_file.read().decode('utf-8')

    elif execution_type == "MGE_experiment":
        MGE_chimeric_fingerprint_file = request.files.get("dataset_chimeric_fingerprint_file")
        MGE_non_chimeric_fingerprint_file = request.files.get("dataset_non_chimeric_fingerprint_file")

        # Verifica che i file siano presenti e validi
        if not MGE_chimeric_fingerprint_file or not MGE_non_chimeric_fingerprint_file:
            return jsonify({"success": False, "message": "Both Chimeric and Non-Chimeric datasets are required."})

        chimeric_content = MGE_chimeric_fingerprint_file.read().decode('utf-8')
        non_chimeric_content = MGE_non_chimeric_fingerprint_file.read().decode('utf-8')
...
def execute_command_ML(command_id):
    ...
    try:
        ...
        if command_id == 1:  # Comando per MGE_experiment.py
            print("Eseguendo Comando 1: MGE Experiment")
            command_args = [
                "python", "MGE_experiment.py",
        ...
        elif command_id == 2:  # Comando per MLL_experiment.py
            print("Eseguendo Comando 2: MLL Experiment")
            command_args = [
                "python", "MLL_experiment.py",
